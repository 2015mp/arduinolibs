...
void SWTFT::flood(uint16_t color, uint32_t len) {
  ...
  if(hi == lo) {
    ...
    while(blocks--) {
      i = 16; // 64 pixels/block / 4 pixels/pass
      do {
        swrite8(hi); swrite8(lo); swrite8(hi); swrite8(lo);
        swrite8(hi); swrite8(lo); swrite8(hi); swrite8(lo);
      } while(--i);
    }
    for(i = (uint8_t)len & 63; i--; ) {
...
void SWTFT::drawHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
	...
	CS_ACTIVE;//SPI.beginTransaction(SPISettings(SPICLOCK, MSBFIRST, SPI_MODE0));
	setAddr(x, y, x+w-1, y);
	...
	CS_IDLE;//SPI.endTransaction();
	
}

void SWTFT::drawVLine(int16_t x, int16_t y, int16_t h,
  uint16_t color)
{
...
void SWTFT::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, 
  uint16_t color) {
...
void SWTFT::writeRegister32(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  swrite8(r);
  CD_DATA;
